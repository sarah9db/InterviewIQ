...
from interview_agents.agents.gmail_agent import fast_reject_node, parse_email_node
...
def build_email_graph():
    ...
    graph.add_node("fast_reject", fast_reject_node)
    ...
    graph.set_entry_point("fast_reject")
    graph.add_conditional_edges(
        "fast_reject",
        lambda state: "end" if state.get("fast_rejected") else "continue",
        {"end": END, "continue": "parse_email"},
    )
